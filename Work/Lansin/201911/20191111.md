...
yahoo 業務難搞 api文件不齊全只好範例轉去建置java環境做測試

eclipse jdk jar import 把正確值做交叉比對

第一關 查出問題 VB 轉行數的保留字碼也被加密
...
![Content-Type: application/json; charset=utf-8](../.gitbook/assets/image%20%2887%29.png)

雅虎購物商誠API文件做成這樣難怪 ...

```text
@費歐娜 不好意思，剛剛花了些時間確定 java 與 改寫後的 .net 所要求的值都是相對的，但是使用postman 對https://tw.supplier.yahoo.com/api/spa/v1/signIn傳值，反傳值如附圖。請問可能有哪些地方需要注意嗎 ?   第二個問題是 請問 api 文件上 header 名稱有兩種，不確定哪個是正確的，有測試兩個都是錯誤的，麻煩您幫忙確認一下，感謝
```
...
public class App 
{
    public static void main( String[] args ) throws JSONException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException
    {

                // Required data
                String timestamp = "1573455721";// Clock.fromNow().getEpochSecondString();
                String token = "Supplier_478";
                int supplierId = 478;
                String secretKey = "yrsW8ukCXsEZOkKmegF7T2WrK0MTq9U2jH3DmSIod24=";
                String iv = "MmThDVTcmRpN5jFgzxb+nQ==";
                String saltKey = "doTzTL6Ev0ttm2OF3kxpzloGTtxky1eI";
                String version = "1";
