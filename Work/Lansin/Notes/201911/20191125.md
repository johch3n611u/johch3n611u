...
後台 input aws s3 串 ajax 連動 select

lerk 叫進去談了一下 主要是隨便聊 感覺對澳洲也沒實質幫助

主要是接下來到離職前的工作分配

還行感覺還可以累積到蠻多東西的但真的就都vb的 ...

然後今天妥協了很多程式碼沒辦法不會用也沒人會 ...

結果問建偉她也覺得vb 麻煩

{% embed url="https://www.one-tab.com/page/WStAda2GQ0WzckVabD2umw" %}
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <table border="1" width="100%" class="doc-table">
            ...
            <tr class="div_attrsetting">
                <td colspan="2">
                    <table border="0" width="100%" class2="doc-table">
                        <tr>
                            <td class="bg" width="80">level 1 屬性:</td>
                            <td colspan="5">
                         ...
                         <tr>
                            <td class="bg" width="80">level 2 屬性:</td>
                            <td colspan="5">
                                <select name="level2" id="level2"></select>
                            </td>
                        </tr>
                         <tr>
                            <td class="bg" width="80">必填其餘屬性:</td>
                            <td colspan="5" name="requiredattr" id="requiredattr">
    ...
    <script>
        ...
        //AJAX小類連動屬性 - 品牌
         $("#catItemId").on("change", function(){
            ...
            $.ajax({
                ...
                data: 'catItemId=' + $("#catItemId").val()+'&attrclass=brand' ,
                dataType: "text",
                success: function (info) {
        ...
        $("#catItemId").on("change", function(){
            ...
            //alert($("#subStationId").val());
            $("#colorname").find("option:selected").text("");
            $("#colorname").empty();
            var attrs;
            $.ajax({
                type: 'POST',
                async: false,  //使用同步
                url: "/mng/Product/list/yahooshoppingscm/ajax_Get_attributes.ashx",
                data: 'catItemId=' + $("#catItemId").val()+'&attrclass=color' ,
                dataType: "text",
                success: function (info) {
        ...
        $("#catItemId").on("change", function(){
            ...
            $.ajax({
                ...
                async: false,  //使用同步
                url: "/mng/Product/list/yahooshoppingscm/ajax_Get_attributes.ashx",
                data: 'catItemId=' + $("#catItemId").val()+'&attrclass=required' ,
                dataType: "text",
                success: function (info) {
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If catItemId <> "" Or catItemId <> Nothing Then
            ...
            Select Case attrclass
                Case "brand"
                    '查品牌屬性
                    Dim attrbrandsql = "SELECT * FROM YahooshoppingSCM_API_struDataAttrClusters WITH (NOLOCK) WHERE catItemId='{0}' AND Attrname='品牌'"
                    attrbrandsql = String.Format(attrbrandsql, catItemId)
                    Dim dtstring = EC.DB.ExecuteDataTable(attrbrandsql).Rows.Item(0).Item(6).ToString

                    '轉為目標格式
                    Dim clear() As Char = {"[", "]"}
                    dtstring = dtstring.Trim(clear)
                    dtstring = dtstring.Replace("""", Nothing)
                    Dim brands() = dtstring.Split(",")

                    '轉為選項
                    Dim attrbrands
                    For num As Integer = 0 To brands.Count - 1 Step 1
                        attrbrands += "<option value='" + brands(num) + "'>" + brands(num) + "</option>"
                    Next

                    context.Response.ContentType = "text/plain"
                    context.Response.Write(attrbrands)


                Case "color"

                    '查品牌屬性
                    Dim attrcolorsql = "SELECT * FROM YahooshoppingSCM_API_struDataAttrClusters WITH (NOLOCK) WHERE catItemId='{0}' AND Attrname='顏色'"
                    attrcolorsql = String.Format(attrcolorsql, catItemId)
                    Dim dtstring = EC.DB.ExecuteDataTable(attrcolorsql).Rows.Item(0).Item(6).ToString

                    '轉為目標格式
                    Dim clear() As Char = {"[", "]"}
                    dtstring = dtstring.Trim(clear)
                    dtstring = dtstring.Replace("""", Nothing)
                    Dim colors() = dtstring.Split(",")

                    '轉為選項
                    Dim attrcolors
                    For num As Integer = 0 To colors.Count - 1 Step 1
                        attrcolors += "<option value='" + colors(num) + "'>" + colors(num) + "</option>"
                    Next

                    context.Response.ContentType = "text/plain"
                    context.Response.Write(attrcolors)

                Case "required"

                    '查必填屬性
                    Dim attrrequiredsql = "SELECT * FROM YahooshoppingSCM_API_struDataAttrClusters WITH (NOLOCK) WHERE catItemId='{0}' AND Attrname not in ('品牌','顏色') AND Attrrequired = 'True' "
                    attrrequiredsql = String.Format(attrrequiredsql, catItemId)
                    Dim dtstring = EC.DB.ExecuteDataTable(attrrequiredsql).Rows.Item(0).Item(6).ToString

                    Dim attrrequireds
                    For num1 As Integer = 0 To dtstring.Count - 1 Step 1


                        attrrequireds += "<input type='text' id='brand' name='brand' value='品牌' readonly='ReadOnly'/>:<select>"
...
![](../.gitbook/assets/gif-20191125-xia-wu-122116.gif)
                ...
                <td colspan="2">
                    <table border="0" width="100%" class2="doc-table">
                        <tr>
                            <td class="bg" width="80">level 1 屬性:</td>
                            <td colspan="5">
                                <select name="brand" id="brand">品牌</select>:
                         ...
                         <tr>
                            <td class="bg" width="80">level 2 屬性:</td>
                            <td colspan="5">
                                <select name="color" id="color">顏色</select>:
                                <select name="colorname" id="colorname"></select>
                            </td>
                        </tr>
                         <tr>
                            <td class="bg" width="80">必填其餘屬性:</td>
