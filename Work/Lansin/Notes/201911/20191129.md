...
![](https://github.com/johch3n611u/EC_Web-AP_Developer/tree/095f673ceb3c1661899447a7223f2f55012c6b3d/.gitbook/assets/image%20%28119%29.png)

上午解決了aws bug 不太確定問題出在哪

後面又出現webform page 架構會檔 html 原始碼 ， 看網路是說要前端加密後端解密， 看以前的人寫太底層了，想說用別種方式查看看

{% embed url="https://smiletrip.pixnet.net/blog/post/341872269-%5B.net%5D%E8%A7%A3%E6%B1%BA%E5%85%B7%E6%9C%89%E6%BD%9B%E5%9C%A8%E5%8D%B1%E9%9A%AArequest.form-%E7%9A%84%E5%80%BC%E5%B7%B2%E5%BE%9E%E7%94%A8" caption="" %}

{% embed url="https://dotblogs.com.tw/chris0920/2011/07/20/31857" caption="" %}

原來之前的人也是關掉處理 ... 去看 web config 才看到 只是有指定那些要關掉
...
他們又寫了個核心做處理 所以 request 都會有別種繼承方法的樣子 有那種 requeststring \( , , \)

![](../.gitbook/assets/image%20%28101%29.png)

{% embed url="http://www.blueshop.com.tw/board/FUM20041006161839LRJ/BRD200305271036247G8.html" caption="" %}

{% embed url="https://blog.xuite.net/jiehui\_prompt/Heal/401975899-VB.net%E3%80%80shared%E5%92%8Cstatic%E9%97%9C%E9%8D%B5%E5%AD%97%E7%9A%84%E5%8D%80%E5%88%A5%E8%A7%A3%E5%AF%86" caption="" %}

遇到問題 多次存取 COOKIES WSSID 後面會不穩 所以一定要將此值轉為靜態 可能 6個小時在存取一次

{% embed url="https://www.codeproject.com/Questions/295229/Static-variable-in-vb-net" caption="" %}

vb At the level of the Class/Module use Shared instead, inside a method/procure use Static.
...
#Region "靜態存取驗證欄位"
    ...
    Public Class YahooRequestDataStatic
        ...
        Public Shared Property YahooResquestData As YahooRequestData
            Get
                Dim TimeEquation = DateDiff("h", Now, _YahooResquestData.Updatetime)
                If (_YahooResquestData Is Nothing) Or (TimeEquation > 4) Then
                    _YahooResquestData = New YahooRequestData With {
                        .Cookies = YahooShoppingSCM_API.Get_YSSCM_CookieCollection()
                    }
                    _YahooResquestData.WSSID = YahooShoppingSCM_API.Get_YSSCM_WSSID_token(_YahooResquestData.Cookies)
                    _YahooResquestData.Updatetime = Now
                End If
                Return _YahooResquestData
            End Get
            Set
...
#Region "YahooShoppingSCM_API_request (使用API)"
    ...
    Public Shared Function Get_YSSCM_API_Response(Method As String, URL As String, JSONString As String)
        ...
        'Try
        '    Using getresponse As HttpWebResponse = CType(request.GetResponse(), HttpWebResponse)
        '        '回傳資料解碼
        '        Using reader2 = New StreamReader(getresponse.GetResponseStream(), Encoding.UTF8)
        '            Dim responseText2 As String = reader2.ReadToEnd()
        '            '回傳API JObject
        '            Dim etag = JsonConvert.DeserializeObject(responseText2).Item("data").Item("eTag").ToString
        '            Return etag
        '        End Using
        '    End Using
        ...
        '    Dim a11 = ex.Data
        '    Dim a12 = ex.HelpLink
        '    Dim a13 = ex.InnerException
        '    Dim a14 = ex.Source
        '    Dim a15 = ex.Message
        'End Try


        Dim testObj = request.GetResponse()
...
{% embed url="https://docs.microsoft.com/zh-tw/dotnet/csharp/programming-guide/xmldoc/returns" caption="" %}
...
{% embed url="https://www.one-tab.com/page/w1jPS1qJRGixOEuCuoOH7Q" caption="" %}

程式碼用檔案傳不放這了 泰卡

![](../.gitbook/assets/image%20%2831%29.png)

{% embed url="https://larry850806.github.io/2016/06/23/regex/" caption="" %}

```text
 {
                "type": "validationError",
                "invalidValue": "KSS AP-1013 可調式配線固定座 (10入)",
                "message": "must match \"^(?!-).{1,25}$\"",
                "messageTemplate": "{javax.validation.constraints.Pattern.message}",
                "path": "SpaResources.createProposal.proposal.listing.seoUrl"
            },
```

{% embed url="https://blog.miniasp.com/post/2010/04/27/How-to-filter-special-characters-using-NET-Regex" caption="" %}

{% embed url="https://docs.microsoft.com/zh-tw/dotnet/visual-basic/language-reference/operators/orelse-operator" caption="" %}
...
Public Class YahooRequestDataStatic
        ...
        Public Shared Property YahooResquestData As YahooRequestData
            Get
                If (_YahooResquestData Is Nothing) OrElse (DateDiff("h", Now, _YahooResquestData.Updatetime) > 4) Then
                    _YahooResquestData = New YahooRequestData With {
                        .Cookies = YahooShoppingSCM_API.Get_YSSCM_CookieCollection()
                    }
                    _YahooResquestData.WSSID = YahooShoppingSCM_API.Get_YSSCM_WSSID_token(_YahooResquestData.Cookies)
