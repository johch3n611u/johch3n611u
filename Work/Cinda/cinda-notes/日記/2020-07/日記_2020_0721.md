...
早上做了難吃的鬆餅，老弟還是叫不醒，倦怠感結果晚了15-30分鐘才來公司...

感覺真的要往獨立接案的最終目標，兼職接案先才能擺脫這種倦怠感...

---

EnabledAnonymous 啟用或停用匿名使用者存取

<https://docs.microsoft.com/zh-tw/skypeforbusiness/manage/federation-and-external-access/access-edge/enable-or-disable-anonymous-user-access>

<https://www.facebook.com/1651384151767194/posts/2149069391998665/>

web api 相關屬性

---

抓近7天有寫入則不寫 ALLOW_DATE / 工號對1筆單

BatchService.cs 參考 排程 , 共用 ? Reposi....
...
<https://docs.microsoft.com/zh-tw/dotnet/api/system.linq.queryable.groupby?view=netcore-3.1>

<https://ithelp.ithome.com.tw/articles/10196181>
...
1. 安裝 Oracle EF
2. 開欄位 Function 401 資安管制 / 新申請系統 / 列管性帳號
3. IS_Holiday 抓環境資料 ???
4. 改 Group 後端，其實要同步 Account 所以 Detail 裡面其實就是 Group 的資料到權限停用申請單 join Account 才會展開
5. 所有有改的地方都要註解規格書第幾頁
6. 6XX 規格怎麼撈，希望這禮拜至少要做到這 <https://docs.google.com/spreadsheets/d/1IC0ZXw2LBW9RBJ4SaBA9l714CdFQ_9GMl2_aJhMrTkc/edit#gid=944457528>
7. SERVICE_NAME 改 FUNCTION_TYPE

---

## 工程師績效評定 一年一次

順博希望工作項目的同時一起達成，團隊分享 ( 工作中遇到的問題issue延伸之類10-20分鐘簡報分享一年兩次... )

<https://docs.google.com/spreadsheets/d/1VylklWD2mgL6AMkgrJFb-3iNX8rZphMy8PWt2JoDP2o/edit#gid=934942037>

---

雖然這個後續不維護，但之前 ICMS 請保哥 CodeReView ，所以盡量不要寫 Hard Code ( 寫死 )

第六循環 ??? 新規格，有問題直接問

---

ODAC for VS 2017 18.3.0.0.0 (67 MB)
...
<https://kevintsengtw.blogspot.com/2011/12/code-snippet-snippet-editor.html>

---

## c# 取當下時間

<http://swaywang.blogspot.com/2012/10/cdatetime-format-string.html>

---

## oracle 吃的 時間格式 2015/01/02

---

## debug 輪子 Entities.Database.Log = s => System.Diagnostics.Debug.WriteLine(s);
...
return Repository.Entities.PAM_IF_RESIGN.Where(x => DateTime.Now.Subtract(x.ACCOUNT_CLOSE_DATE).Days > 7 ).ToList().Count;
...
void Main()
{
	TimeSpan SevenDays = new TimeSpan(7, 0, 0);
	Console.WriteLine(SevenDays);
}

//TimeSpan SevenDays = new TimeSpan(7, 0, 0);
            //return Repository.Entities.PAM_IF_RESIGN.Where(x => (x.ACCOUNT_CLOSE_DATE - DateTime.Now) < SevenDays).ToList().Count;

            return Repository.Entities.PAM_IF_RESIGN.Where(x => (x.ACCOUNT_CLOSE_DATE.Day - DateTime.Now.Day) < 7).ToList().Count;

---

## any count ?

<https://dotblogs.com.tw/regionbbs/2013/04/10/when_to_use_any_or_count_in_linq>
